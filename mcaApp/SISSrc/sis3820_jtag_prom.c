...
/* Modified by Mark Rivers, University of Chicago
 * Changed to run on vxWorks and RTEMS rather than Linux with PCI/VME bridge.
 */
...
static unsigned char *gl_wbuf;
static unsigned char *gl_rbuf;
...
static int correct_byte_order(int data);


static int jtag_program_verifier (unsigned int base_addr, char* fname, int program) ;

static int vme_A32D32_read(int addr, int *data);
static int vme_A32D32_write(int addr, int data);

/****************************************************************************/


int sis3820_jtag_prom(epicsUInt32 baseAddress, const char *cmd_parameter_ptr, char *mcs_file_name_ptr)
{
   /* command_parameter_ptr = "R" (Read IDCode)
        ...
        case 'R' : { /* data record */
            ...
            for (i=0;i<gl_jtg_nof_devices;i++)  {
                ...
                if (((gl_jtg_dev_idcodes[i] & 0x0fffffff) == 0x05026093) ||
                    ((gl_jtg_dev_idcodes[i] & 0x0fffffff) == 0x05036093)) {
                    printf("device = 18V04 \n");
                }
                else {
...
{
  int status;
  status = devReadProbe(4, (char *) addr, (char *) data);
  if (status) {
    printf("vme_A32D32_read: status=0x%08x at addr=0x%08x\n",status,addr);
  }
  return status;
}
...
{
  int status;
  status = devWriteProbe(4, (char *) addr, (char *) &data);
  if (status) {
    printf("vme_A32D32_write: status=0x%08x at addr=0x%08x\n",status,addr);
  }
  return status;
}
...
static const iocshFuncDef configFuncDef = 
    {"sis3820_jtag_prom",3,configArgs};

static void configCallFunc(const iocshArgBuf *args)
{
    sis3820_jtag_prom(args[0].ival, args[1].sval, args[2].sval);
}
...
/* enable      */
  ...
  if (vme_A32D32_write(addr, 0x1)) return -1 ;
  ...
  do {
     uy=0;
     do { 
        if (vme_A32D32_write(addr, 0x2)) return -1 ;
     } while (++uy < 5);
        if (vme_A32D32_write(addr, 0x0)) return -1 ;
   } while (++ux < 3);
...
int jtag_instruction(unsigned int base_addr, unsigned int icode)
{
  ...
  addr = base_addr + SIS3820_JTAG_TEST ;
  if (vme_A32D32_write(addr, 0x3)) return -1 ;
  if (vme_A32D32_write(addr, 0x3)) return -1 ;
  if (vme_A32D32_write(addr, 0x1)) return -1 ;
  if (vme_A32D32_write(addr, 0x1)) return -1 ;
  ...
  while (ux) {
    ...
    do {
       ...
       if (ux != gl_jtg_select) {
          addr = base_addr + SIS3820_JTAG_TEST ;
          if (vme_A32D32_write(addr, ms |1)) return -1 ;
         } else {
          addr = base_addr + SIS3820_JTAG_DATA_IN ;
          if (vme_A32D32_read(addr, &data)) return -1 ;
                tdo |=((data >>31) &1) <<uy;
          addr = base_addr + SIS3820_JTAG_TEST ;
          if (vme_A32D32_write(addr, ms |(icode &1))) return -1 ;
            icode >>=1;
       }
...
int jtag_program_verifier (unsigned int base_addr, char* fname, int program)
{
    ...
    if (((gl_jtg_dev_idcodes[0] & 0x0fffffff) != 0x05026093) &&
        ((gl_jtg_dev_idcodes[0] & 0x0fffffff) != 0x05036093)) {
        printf("\n\nERROR:   no 18V04   \n\n\n");
        return -1 ;
    }
    ...
    while (rest_read_length != 0) {
        ...
        printf("rest_read_length  = %08X            %03d%%  \r",
            rest_read_length, ((100*read_byte_index)/mcs_buffer_length));
