...
/* Interface supported by asynIcbDriver  */
#define asynIcbType "asynIcb"

/* NOTE: the following values must match the CARD values in the database
   (.db) files */
typedef enum {
    icbAdcType,
    icbAmpType,
...
/* NOTE: the following values must match the SIGNAL values in the database 
 * (.db) files */
typedef enum {
...
typedef struct {
    asynStatus (*read)(void *drvPvt, asynUser *pasynUser,
                       ...
                       int icbCommand, int *ivalue, double *dvalue);
